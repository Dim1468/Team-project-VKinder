...
1. Бот инициализируется с помощью токена сообщества и токена пользователя.
2. Основной цикл main_loop() слушает события новых сообщений и выполняет соответствующие действия в зависимости от полученного текста сообщения.
3. При получении команды "Начать" бот отправляет пользователю клавиатуру с кнопками "Мужчина" и "Женщина".
4. При выборе пола бот отправляет пользователю клавиатуру с кнопкой "Далее".
5. При нажатии на кнопку "Далее" бот отправляет пользователю сообщение с запросом ввода возраста.
6. После ввода возраста бот отправляет пользователю сообщение с запросом ввода города.
7. После ввода города бот выполняет поиск пользователей по заданным критериям (пол, возраст, город) с помощью функции search_users(user_id, criteria, user_token).
8. Результаты поиска пользователей бот отправляет пользователю в виде сообщений.

### Подробное описание представлено в разделе "Инструкция по использованию бота"
# Инструкция по использованию бота

## Описание

[Подробнее о взаимодействии с VK ботом](documentation interaction with the VK bot.md)

## Интеграция с базой данных

[Подробнее о классе User_DB](documentation integration with data base.md)

## Интеграция бота и базы данных

[Подробнее о классе VK_Bot](documentation integration of bot and database.md)

## Схема БД

![Схема БД](PSQL_tables2.png)
